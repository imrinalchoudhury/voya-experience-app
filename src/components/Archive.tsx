...
export function Archive({ onJourneySelect }: ArchiveProps) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-voya-dark pt-32 px-8">
  ...
  return (
    <div className="min-h-screen bg-voya-dark pt-32 px-8 pb-20">
      <div className="max-w-7xl mx-auto">
        <div className="mb-16 text-center">
          <div className="font-montserrat text-voya-gold/60 text-[9px] tracking-[3px] uppercase mb-6">
            Travel Memoir
          </div>
          <h1 className="font-cormorant text-[64px] leading-[1.1] font-light">
            Your story,
            <br />
        ...
        {journeys.length === 0 ? (
          <div className="text-center py-24">
            <p className="font-cormorant text-3xl font-light text-voya-gold/40 mb-3">
              No completed journeys yet.
            </p>
            <p className="font-cormorant text-xl font-light text-voya-gold/30">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {journeys.map((journey, index) => (
              <div
                ...
                className="group relative bg-voya-card border border-voya-gold/20 p-8 cursor-pointer transition-all duration-500 hover:border-voya-gold/60 opacity-85 hover:opacity-100"
                style={{
                  animation: `fadeUp 0.6s ease-out ${index * 0.1}s both`,
                }}
              >
                <div className="absolute top-4 right-4">
                  <div className="font-montserrat text-[8px] tracking-[2px] text-voya-gold/30 uppercase">
                    Completed
                  </div>
                </div>

                <div className="mb-6">
                  <div className="font-montserrat text-[9px] tracking-[3px] text-voya-gold/40 uppercase mb-2">
                    {journey.category}
                  </div>
                  <h2 className="font-cormorant text-3xl font-light text-white mb-2 group-hover:text-voya-gold transition-colors">
                    {journey.title}
                  </h2>
                  <p className="font-cormorant text-xl italic text-voya-gold/60 mb-4">
                    {journey.tagline}
                  </p>
                </div>

                <div className="h-px bg-gradient-to-r from-transparent via-voya-gold/20 to-transparent mb-6"></div>

                <div className="space-y-3">
                  <div className="flex items-baseline justify-between">
                    <span className="font-montserrat text-[9px] tracking-[2px] text-voya-gold/40 uppercase">
                      Destination
                    </span>
                    <span className="font-montserrat text-sm text-voya-gold/60">
                      {journey.destination}
                    </span>
                  </div>
                  <div className="flex items-baseline justify-between">
